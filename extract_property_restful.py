...
class ExtractShoesProperty(base_model.HttpModel):
    """extract shoes goods property"""
    ...
    def predict(self, uttrance):
        ...
        results = requests.post(url=self.predict_url, json=features)
        results = json.loads(results.text)
        start_points = np.asarray(results["outputs"]["start_points"]).reshape(-1, len(self.property_dict))
        end_points = np.asarray(results["outputs"]["end_points"]).reshape(-1, len(self.property_dict))
